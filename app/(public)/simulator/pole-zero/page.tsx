...
export default function PoleZeroSimulator() {
    ...
    return (
        ...
        <span className="font-mono text-meta text-text-muted">Poles</span>
        <select className={`${selectCls} mt-2`} value={numPoles} onChange={e => setNumPoles(+e.target.value)}>
        {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        </label>
        <label className="flex flex-col">
        <span className="font-mono text-meta text-text-muted">Zeros</span>
        <select className={`${selectCls} mt-2`} value={numZeros} onChange={e => setNumZeros(+e.target.value)}>
        ...
        {/* tablet:, not md: — --breakpoint-*: initial removed Tailwind's defaults,
            ...
            //
            // A zero drives |H(s)| to exactly 0 at its location — that's already
            // in the data, it just used to be invisible because 0 landed on --bg
            // and blended into the surrounding low-magnitude navy. Rather than a
            // separate marker trace, the bottom of the ramp now spikes to --accent
            // and falls back to --bg within a couple percent of the range, so a
            // zero reads as a thin red ring burned into the surface itself. Poles
            // stay the only thing that reaches gold, so the two roots can't be
